...
### Исправлено
- **Ложные срабатывания детекции телефонов на фото** (`bot/utils/content_filter.py`)
  ...
  - Теперь блокируются только реальные телефоны, а не любые 10-11 цифр
- **Убрана двойная проверка фото** (`bot/handlers/ad_creation.py`)
  - Проверка теперь только при загрузке фото (кнопка "Далее"), а не при публикации
  - Ускорена публикация объявлений

### Добавлено
- **Локальная модерация изображений** — без облачных лимитов, быстрая проверка
  - Новый сервис `bot/services/local_moderation.py`
  - **EasyOCR** — распознавание текста на фото (русский + английский)
  - **NudeNet** — детекция NSFW-контента (обнажёнка)
  - Параллельная проверка OCR + NSFW через ThreadPoolExecutor
  - Скорость: ~1 сек/фото (против 1 req/sec лимита у Yandex Vision)
  - Нет лимитов — можно обрабатывать много фото одновременно
  - Yandex Vision OCR оставлен как fallback
  - Настройки: `LOCAL_MODERATION_ENABLED`, `LOCAL_MODERATION_NSFW_THRESHOLD` в settings.py

### Изменено
- **Проверка фото при публикации** (`bot/handlers/ad_creation.py`)
  - Приоритет: сначала локальная модерация, потом Yandex Vision
  - Добавлена проверка NSFW-контента на фото
  - Обновлена функция `check_photos_for_forbidden_text()`
