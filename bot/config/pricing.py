...
"""
Конфигурация подписок, лимитов и цен.

ЛЕГКО РЕДАКТИРУЕТСЯ без изменения кода.
В будущем — вынести в админку.

Изменение цен и лимитов:
1. Отредактировать нужные значения в этом файле
2. Перезапустить бота: supervisorctl restart telegram_bot
"""

from typing import Dict, Any, Optional

# =============================================================================
# ТИПЫ АККАУНТОВ / ПОДПИСКИ
# =============================================================================
...
ACCOUNT_TYPES: Dict[str, Dict[str, Any]] = {
    "free": {
        ...
        "limits": {
            ...
            "video_allowed": False,          # Видео в объявлении
...
PAID_SERVICES: Dict[str, Dict[str, Any]] = {
    ...
    "ad_extend_7d": {
        ...
        "description": "Продление срока публикации объявления",
    ...
    # -------------------------------------------------------------------------
    # Дополнительные возможности объявления
    # -------------------------------------------------------------------------
    "ad_multiregion": {
        "name": "Дополнительный регион",
        "description": "Публикация объявления в дополнительном регионе",
        "category": "ad_feature",
        "price_rub": 19,
        "per_item": True,  # Цена за каждый доп. регион
        "is_active": True,
    },
    "ad_extra_link": {
        "name": "Дополнительная ссылка",
        "description": "Добавить ещё одну ссылку к объявлению",
        "category": "ad_feature",
        "price_rub": 9,
        "per_item": True,
        "is_active": True,
    ...
    "ad_video": {
        "name": "Видео в объявлении",
        "description": "Возможность добавить видео к объявлению",
        "category": "ad_feature",
        "price_rub": 29,
        "is_active": True,
    },
    ...
    "pin_channel_1h": {
        "name": "Закреп в канале 1 час",
        "description": "Закрепить объявление в канале на 1 час",
        "category": "promotion",
        "price_rub": 49,
        "duration_hours": 1,
        "is_active": True,
    ...
    "pin_channel_3h": {
        "name": "Закреп в канале 3 часа",
        "description": "Закрепить объявление в канале на 3 часа",
        "category": "promotion",
        "price_rub": 99,
        "duration_hours": 3,
        "is_active": True,
    ...
    "story_publish": {
        "name": "Публикация в Stories",
        "description": "Опубликовать объявление в Stories канала",
        "category": "promotion",
        "price_rub": 79,
        "duration_hours": 24,
        "is_active": True,
    ...
    "badge_urgent": {
        "name": "Бейдж «Срочно»",
        "description": "Добавить метку срочности к объявлению",
        "category": "promotion",
        "price_rub": 29,
        "duration_hours": 7 * 24,
        "is_active": True,
    ...
    "btn_call": {
        ...
        "description": "Добавить кнопку быстрого звонка",
        "category": "promotion",
        "price_rub": 19,
        "is_active": True,
    },
    ...
    # -------------------------------------------------------------------------
    "banner_between_ads": {
        "name": "Баннер между объявлениями",
        "description": "Рекламный баннер между объявлениями в канале",
    ...
    "ads_pack_10": {
        "name": "+10 объявлений",
        "description": "Увеличить лимит активных объявлений на 10",
        "category": "account",
        "price_rub": 49,
        "ads_count": 10,
        "duration_hours": 30 * 24,
    ...
    "ads_pack_50": {
        "name": "+50 объявлений",
        "description": "Увеличить лимит активных объявлений на 50",
        "category": "account",
        "price_rub": 199,
        "ads_count": 50,
        "duration_hours": 30 * 24,
...
# =============================================================================
# КАТЕГОРИИ УСЛУГ (для отображения в меню)
# =============================================================================

SERVICE_CATEGORIES = {
    "ad_duration": {
        "name": "📅 Продление",
        "description": "Продлить срок публикации",
        "order": 1,
    },
    "ad_feature": {
        "name": "✨ Возможности",
        "description": "Дополнительные возможности объявления",
        "order": 2,
    },
    "promotion": {
        "name": "🚀 Продвижение",
        "description": "Продвижение объявления",
        "order": 3,
    },
    "advertising": {
        "name": "📢 Реклама",
        "description": "Рекламные размещения",
        "order": 4,
    },
    "account": {
        "name": "👤 Аккаунт",
        "description": "Услуги для аккаунта",
        "order": 5,
    },
}
...
AD_LIFECYCLE_CONFIG = {
    # Продление объявления (кнопка "Продлить" в уведомлении)
    "extend": {
        "is_free": True,                   # Продление бесплатное
        "duration_days": 30,               # На сколько дней продлевается
    },

    # Переопубликация (когда объявление в архиве, кнопка "Восстановить")
    "republish": {
        "free_first_time": True,           # Первая переопубликация бесплатна
        "price_rub": 29.0,                 # Цена переопубликации (если платно)
        "price_stars": 15,                 # Цена в Stars
        "cooldown_hours": 24,              # Минимум часов между переопубликациями
    },

    # Уведомления об истечении срока публикации
    # Отправляются с кнопками "Продлить" и "Снять"
    "notifications": {
        "enabled": True,
        "warn_days": [2, 1],               # За сколько дней уведомлять (28 и 29 день)
        "final_warn_hours": 1,             # За сколько часов до удаления (30 день)
        "send_time_hour_msk": 10,          # Время отправки уведомлений (МСК)
    },

    # Архивация неактивных объявлений
    "archive": {
        "inactive_retention_days": 90,     # Сколько дней хранить неактивные (3 месяца)
        "cleanup_batch_size": 100,         # Сколько объявлений обрабатывать за раз
    },
}
...
STARS_CONFIG = {
    "usd_multiplier": 0.013,    # 1 Star = $0.013 (официальный курс Telegram)
    "discount": 0.9,            # Коэффициент (0.9 = скидка 10%)
    "min_rate_rub": 1.0,        # Минимум 1₽ за звезду
    "update_hour_msk": 4,       # Час обновления курса (МСК)
    "fallback_usd_rub": 90.0,   # Курс USD/RUB по умолчанию если API недоступен
}
...
#
# Пример при USD/RUB = 90:
# star_rub = 90 * 0.013 * 0.9 = 1.053 ₽
...
PROMO_TYPES = {
    "fixed_rub": {
        ...
        "description": "Скидка на фиксированную сумму в рублях",
    },
    "percent": {
        "name": "Процент скидки",
        "description": "Скидка в процентах от суммы",
    },
    "bonus_rub": {
        "name": "Бонус в рублях",
        "description": "Начисление бонуса на баланс в рублях",
    },
    "bonus_stars": {
        "name": "Бонус в Stars",
        "description": "Начисление бонуса на баланс в Stars",
    },
    "free_service": {
        "name": "Бесплатная услуга",
        "description": "Одна бесплатная услуга",
    },
}
...
def get_account_limits(account_type: str) -> Dict[str, Any]:
    """Получить лимиты для типа аккаунта"""
...
def get_active_services(category: Optional[str] = None) -> Dict[str, Dict]:
    """Получить активные услуги, опционально по категории"""
    services = {}
